...
export default function HostDashboard() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(false)
    if (!res.ok) { setParseError(data.error || 'Could not parse PDF'); return }
  ...
  async function handleCreate() {
    ...
    const data = await res.json()
    if (!res.ok) { alert(data.error || 'Failed to create game'); setCreating(false); return }
  ...
  return (
    <div className="min-h-screen bg-cobalt-900 px-4 py-6">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {/* ── STEP 1: Upload ──────────────────────────────────── */}
        {step === 'upload' && (
          <div className="bg-cobalt-800 border-2 border-cobalt-600 rounded-2xl p-8">
            <h2 className="font-display text-3xl text-gold-400 tracking-wide mb-2">
              🥚 New Easter Quiz
            </h2>
            <p className="text-white/60 mb-8">
              Upload your questions PDF. Each question needs Q1. Q2. format with A) B) C) options and an Answer: line.
            </p>

            <div
              onClick={() => fileRef.current?.click()}
              className="border-4 border-dashed border-cobalt-600 hover:border-gold-400
                         rounded-2xl p-16 text-center cursor-pointer transition-colors group"
            >
              <div className="text-6xl mb-4">
                {uploading ? '🐣' : '📄'}
              </div>
              <p className="font-display text-2xl text-gold-400 group-hover:text-gold-300 tracking-wide">
                {uploading ? 'Cracking open the PDF…' : 'Click to upload PDF'}
              </p>
              <p className="text-white/40 text-sm mt-2">Up to 10 questions, 3 or 4 options each</p>
            </div>
            ...
            {parseError && (
              <div className="mt-4 p-4 bg-red-900/40 border border-red-500 rounded-xl text-red-300 text-sm">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-display text-3xl text-gold-400 tracking-wide">
                🐣 {questions.length} Question{questions.length !== 1 ? 's' : ''} — Preview
              </h2>
              <button
            ...
            <div className="space-y-4 mb-8">
              {questions.map((q) => (
                <div
                  ...
                  className="bg-cobalt-800 border-2 border-cobalt-600 rounded-2xl p-6"
                >
                      ...
                      return (
                        <div
                          ...
                          className={`px-3 py-2 rounded-xl text-sm border-2 ${
                            isAnswer
                              ? 'border-easter-green bg-green-900/30 text-green-300 font-semibold'
            ...
            <button
              ...
              className="w-full bg-gold-400 hover:bg-gold-300 disabled:opacity-50
                         ...
                         py-5 rounded-2xl transition-colors uppercase"
            >
              {creating ? '🐰 Creating…' : '🐰 Start the Easter Hunt →'}
            </button>
          </div>
        )}
      </div>
